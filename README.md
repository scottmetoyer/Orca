...
This is a fork of Orca adding QR code creation and reading features.

To create a QR code from a selection, select a block of Orca code and choose File->Export Selection as QR Code from the menu. A PNG file containing a QR code will be created from the selection.

To scan an existing QR code into Orca, choose File->Scan QR Code from the menu. Make sure the Orca window has focus and scan the code using a compatible reader. The Orca code saved in the scan will be loaded into Orca.

A scan can be canceled by pressing the Escape key.

Any QR code scanner that behaves like a keyboard should work here. Make sure the scanner is configured to send a carriage return after a scan (this behavior is on by default in many scanners). A <a href="https://www.amazon.com/gp/product/B07LG4TBVZ">NADAMOO scanner from Amazon</a> was used for testing.
